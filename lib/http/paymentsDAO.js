...
export async function getPaymentsByOrderId(orderId) {
	...
	try {
		const payments = await Payments.find({ orderId: orderId }).sort({ date: -1 }).exec();
		return payments;
	} catch (error) {
		console.error(error);
 ...
 */
export async function recordNewCardPayment(paymentIntent, orderId, card, orderState = '', invoiceId = null) {
	await dbConnect();
	logServerDatabaseCall('lib/http/paymentsDAO/recordNewCardPayment', ...arguments);

	const username = cookies().has('user') ? JSON.parse(cookies().get('user').value).username : 'CUSTOMER';
