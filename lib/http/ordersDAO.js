...
export async function attachNewPayment(orderId, paymentId, charge) {
	...
	try {
		...
		if (!(order.text.additionalDescription)) {
			additionalUpdates['text.additionalDescription'] = process.env.CURRENT_TERMS_AND_CONDITIONS_FOR_INSTALL_ORDER;
		}

		if (Object.keys(additionalUpdates).length) {
			await Orders.fineOneAndUpdate({
				_id: orderId
			}, additionalUpdates, {
				upsert: false
			});
