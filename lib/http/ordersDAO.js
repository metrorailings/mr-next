...
export async function saveNewOrder(order) {
	...
	order.modHistory = [];
	order.balanceRemaining = order.pricing.orderTotal || 0;
...
export async function saveChangesToOrder(order, modHistoryRecord = updateModHistory()) {
	...
	try {
		const savedOrder = await Orders.findOneAndUpdate({
			...
			$push: { modHistory: modHistoryRecord }
		},{
			upsert: false
		}).exec();
		return savedOrder;
...
/**
 * Function responsible for denoting within an order that a quote has been issued and udpating the status accordingly
 *
 * @param {Object} order
 */
export async function recordNewQuoteDraft(order) {
	await dbConnect();
	logServerDatabaseCall('lib/http/ordersDAO/recordNewQuoteDraft', ...arguments);

	let updateData = {
		status: (getStatusMetadata(status).state === 'prospect') ? quoteStatus.key : status
	};
	...
	try {
		const updatedOrder = await Orders.findOneAndUpdate({
			...
			...updateData,
			$inc: { 'sales.quoteSeq': 1 },
			$push: { 'sales.quotes' : order._id + '-' + (order.sales.quoteSeq + 1) }
		}, {
			upsert: false
		}).exec();
