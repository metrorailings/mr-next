...
/**
 * Function designed to send an e-mail informing a prospect that a quote is ready for him
 */
export async function sendQuoteEmail(order) {
	...
	data.id = order._id;
	data.productDesc = order.productHeader;
	data.name = order.customer.name;
	data.company = order.customer.company || '',
	...
	for (let i = 0; i < order.customer.email.length; i += 1) {
		const postData = {
			...
			transactionalId: process.env.LOOPS_QUOTE_EMAIL_TRANSACTION_ID,
			dataVariables: data
		};
